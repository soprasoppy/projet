# Kanged From Techno Mindz @TmMainChannel
import asyncio
import re
import ast
...
async def next_page(bot, query):
    ...
    if int(req) not in [query.from_user.id, 0]:
        return await query.answer("Efectuez votre propre requête 😤", show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0
    ...
    if not search:
        await query.answer("Vous utilisez mes anciens messages🥲,essayez de demander à nouveau 🤠.", show_alert=True)
        return
    ...
    if n_offset == 0:
        ...
        btn.append(
            [InlineKeyboardButton(f"🗓 {round(int(offset) / 10) + 1} / {round(total / 10)}", callback_data="pages"),
             InlineKeyboardButton("Suiv.. ⏩", callback_data=f"next_{req}_{key}_{n_offset}")])
    else:
        btn.append(
            [
                InlineKeyboardButton("⏪ RETOUR", callback_data=f"next_{req}_{key}_{off_set}"),
...
async def advantage_spoll_choker(bot, query):
    ...
    if not movies:
        return await query.answer("Vous cliquez sur un ancien bouton qui a expiré.", show_alert=True)
    movie = movies[(int(movie_))]
    await query.answer('Recherche du film dans la BD...')
    ...
    if k == False:
        ...
        if files:
            ...
            k = await query.message.edit('Ce film est introuvable dans ma BD')
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        if chat_type == "private":
            ...
            if grpid is not None:
                ...
                try:
                    ...
                    return await query.answer('Que cherchez-vous 🤔')
            else:
                await query.message.edit_text(
                    "Je ne suis connecté à aucun groupe!\nVérifier /connections ou connectez-vous à n’importe quel groupe",
                    quote=True
                )
                return await query.answer('Que cherchez-vous 🤔')
        ...
        if (st.status == "creator") or (str(userid) in ADMINS):
            ...
            await query.answer("Vous devez être propriétaire du groupe ou utilisateur Auth pour ce faire. -_- !", show_alert=True)
        ...
        if mkact:
            ...
            await query.message.edit_text('Une erreur s’est produite!!', parse_mode="md")
        return await query.answer('Que cherchez-vous 🤔')
    ...
    if query.data.startswith("file"):
        ...
        try:
            if AUTH_CHANNEL and not await is_subscribed(client, query):
                ...
                await query.answer('Vérifiez PM, j,ai envoyé les fichiers en PM', show_alert=True)
        except UserIsBlocked:
            await query.answer('Débloquer le bot Super Man 😑!', show_alert=True)
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await query.answer("J’aime ton intelligence, mais ne sois pas trop intelligent 😒", show_alert=True)
            return
        ident, file_id = query.data.split("#")
        files_ = await get_file_details(file_id)
        if not files_:
        ...
        buttons = [[
            ...
            InlineKeyboardButton('🤖 𝗡𝗼𝘂𝘃𝗲𝗹𝗹𝗲𝘀', url='https://t.me/belleflaura')
        ], [
        ...
        await query.answer('🏠𝐀𝐜𝐜𝐮𝐞𝐢𝐥...')
    elif query.data == "help":
        buttons = [[
            InlineKeyboardButton('Filtre Manuel', callback_data='manuelfilter'),
            InlineKeyboardButton('Filtre Auto', callback_data='autofilter')
        ], [
            InlineKeyboardButton('Connection', callback_data='coct'),
            InlineKeyboardButton('Extra Mods', callback_data='extra')
        ], [
            InlineKeyboardButton('🏠 𝐀𝐜𝐜𝐮𝐞𝐢𝐥', callback_data='start'),
            InlineKeyboardButton('🔮 𝐒𝐭𝐚𝐭𝐮𝐭', callback_data='stats')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        r=await query.message.reply_text('▣▣▢▢▢▢')
        a=await r.edit('▣▣▣▢▢▢')
        ...
        await asyncio.sleep(1)
        await n.delete()
        await query.answer("Bienvenue dans mon module d’aide")
        await query.message.edit_text(
        ...
        await asyncio.sleep(1)
        await n.delete()
        await query.answer("Voir des Infos sur moi 😉")
        await query.message.edit_text(
        ...
        buttons = [[
            ...
            InlineKeyboardButton('⏹️ ʙᴏᴜᴛᴏɴs', callback_data='button')
        ]]
        r=await query.message.reply_text('▣▣▢▢▢▢')
        a=await r.edit('▣▣▣▢▢▢')
        v=await a.edit('▣▣▣▣▢▢')
        ...
        await asyncio.sleep(1)
        await n.delete()
        await query.answer("Chargement du filtre manuel...")
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        await asyncio.sleep(1)
        await n.delete()
        await query.answer("Chargement du Module Boutton...")
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        await asyncio.sleep(1)
        await n.delete()
        await query.answer("Chargement du filtre automatique...")
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        await asyncio.sleep(1)
        await n.delete()
        await query.answer("Vérifier Mes Mondules de connexions...")
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
