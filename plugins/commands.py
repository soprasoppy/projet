...
async def start(client, message):
    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        ...
        btn = [
            [
                InlineKeyboardButton(
                    "🤖 Rejoindre le canal des Nouvelles", url=invite_link.invite_link
                )
            ]
        ]
        ...
        if message.command[1] != "subscribe":
            try:
            	...
            	btn.append([InlineKeyboardButton(" 🔄 Réessayez", callback_data=f"{pre}#{file_id}")])
            except IndexError:
                btn.append([InlineKeyboardButton(" 🔄 Réessayez", url=f"https://t.me/{temp.U_NAME}/{message.command[1]}")])
        await client.send_message(
            chat_id=message.from_user.id,
            text="**Veuillez rejoindre mon canal des Nouvelles pour utiliser ce bot!**",
    ...
    if data.split("-", 1)[0] == "BATCH":
        sts = await message.reply("Veuillez patienter")
    ...
    if not files_:
        ...
        try:
            ...
            pass
        return await message.reply('Aucun fichier de ce type n’existe.')
    files = files_[0]
    ...
    await message.reply_text(
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        text="⚠️DÉTRUIRE⚠️", callback_data="autofilter_delete"
                    )
                ],
                [
                    InlineKeyboardButton(
...
async def delete_all_index_confirm(bot, message):
    ...
    await message.answer('Que cherchez-vous 🤔')
